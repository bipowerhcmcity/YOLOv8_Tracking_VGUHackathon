...
class StrongSORT(object):
    def __init__(self, 
        ...
        self.init_predefined_id()
    def init_predefined_id(self):
        img_crop = []
        for img_path in glob.glob("predefined_id/*.PNG"):
            img = cv2.imread(img_path)
            img_crop.append(img)
            print(img.shape)
        features = self.model(img_crop)
        detections = [Detection([0,0,img_crop[i].shape[1],img_crop[i].shape[0]], 1, feature) for i, feature in enumerate(
            features)]
        for detection in detections:
            self.tracker._initiate_track(detection,2,1,14)
